...
if ($cdata_status == 'pending') {
    ...
    echo '<h3 class="account__company-customer-title">مشتریان</h3>';
    echo '</div>';
    echo '<div class="account__company-customer-content">';

    $rejection_reason = get_field('rejection_reason_customer', $com_id);
    ...
    $formsetting = array(
        ...
        <input type="hidden" id="acf-field_65084a64639af" name="acf[field_65084a64639af]" value="draft"/>
        ',
    ...
    jQuery(document).ready(function($) {
        ...
        $('#btnsend').on('click', function() {
            if (!confirm("آیا مطمئن هستید؟ (بعد از تایید اطلاعات این فرم جهت بازبینی و انتشار در سایت برای مدیر ارسال خواهد شد.)")) {
                ...
                event.preventDefault();
                $('#acf-field_65084a64639af').val('pending');
                $('form#cform').submit();
            }
        });
    });
